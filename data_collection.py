...
class NumberPlateCollector:
    ...
    def _update_labels(self, candidate_label, overlap_index):
        candidate_screen_time = self._get_time_on_screen(candidate_label)
        overlap_screen_time = self._get_time_on_screen(
            self.total_data[overlap_index]['numberplate'])

        if candidate_screen_time > overlap_screen_time:
            self.total_data[overlap_index]['numberplate'] = candidate_label
    ...
    def _update_time_on_screen(self, label):
        ...
        if label not in self.times_on_screen:
            self.times_on_screen[label] = {}
            self.times_on_screen[label]['start_time'] = time.time()
        elif start_time - self.times_on_screen[label]['end_time'] > 30:
            self.times_on_screen[label]['start_time'] = time.time()
